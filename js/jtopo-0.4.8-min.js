	...
	function c(c) {
		...
		this.initialize = function(c) {
			m(c), this.canvas = c, this.graphics = c.getContext("2d"), this.childs = [], this.frames = 24, this.messageBus =
				new a.util.MessageBus, this.eagleEye = b(this), this.scalePercent = 0, this.wheelZoom = null, this.mouseDownX = 0,
				this.mouseDownY = 0,
...
function(a) {
	function b(c) {
		...
		return f.forEach(function(a) {
			...
			this.stage.scalePercent++;
			if (this.stage.scalePercent > 0) {
				$('.scalePercent').html(`放大 ${this.stage.scalePercent*10}%`)
			} else if (this.stage.scalePercent === 0) {
				$('.scalePercent').html(`原始大小`)
			} else {
				$('.scalePercent').html(`缩小 ${this.stage.scalePercent*10}%`)
			}
			$('.scalePercent').fadeIn(1000, function() {
				$(this).fadeOut(1000)
			})
...
function(a) {
	function b(c) {
		this.initialize = function(c) {
			...
			if (this.image) {
				var b = a.globalAlpha;
				a.globalAlpha = this.alpha, null != this.alarmImage && null != this.alarm ? a.drawImage(this.alarmImage, -this.width /
			...
			if ("string" == typeof a) {
				...
				null == d ? (d = new Image, d.src = a, d.onload = function() {
					j[a] = d, 1 == b && c.setSize(d.width, d.height), c.image = d, c.alarmColor = null == c.alarmColor ? "255,0,0" :
						c.alarmColor
				}) : (b && this.setSize(d.width, d.height), c.image = d, c.alarmColor = null == c.alarmColor ? "255,0,0" : c.alarmColor)
			} else this.image = a, c.alarmColor = null == c.alarmColor ? "255,0,0" : c.alarmColor, 1 == b && this.setSize(a.width,
				a.height)
	...
	function d(a) {
		this.initialize(), this.text = a, this.elementType = "TextNode", this.paint = function(a) {
			a.beginPath(), a.font = this.font, this.width = a.measureText(this.text).width, this.height = a.measureText("田").width,
...
function(a) {
	...
	function f(b, c, g) {
		...
		this.initialize = function(b, c, d) {
			...
			b.beginPath(), b.fillStyle = "rgba(" + this.strokeColor + "," + this.alpha + ")", 
			b.moveTo(o.x, o.y), 
			b.lineTo(m, n),
			b.lineTo(p.x, p.y), 
			b.fillStyle='#ffc814'; //以上面定义的渐变填充
			b.fill();
			// b.stroke(), 
			// b.closePath()
		}, this.paint = function(a) {
		...
		this.initialize = function() {
			...
			for (var c = 1; c < b.length; c++) {
				...
				g += (e.y - d.y) / 2, a.strokeStyle = "rgba(" + this.strokeColor + "," + this.alpha + ")", a.lineWidth = this.lineWidth,
					...
					if(d.y-e.y<26 && d.y-e.y>-26){  //两个节点是平行的 变成直线
						a.lineTo(e.x, e.y)
						a.closePath()
					}else if(d.x-e.x>56 || d.x-e.x<-56 ){ //两个节点不是垂直的时候
						a.bezierCurveTo(d.x,g,e.x,f,e.x,e.y)
					}else if(d.x-e.x<36 || d.x-e.x>-36 ){//两个节点是垂直的时候 画弧线
						a.quadraticCurveTo(f, g, e.x, e.y)
					}else if(d.x-e.x<16 || d.x-e.x>-16 ){
						a.lineTo(e.x, e.y)
						a.closePath()
					}
					a.stroke()
