...
var model_right_needId = ''; //全局变量, 右侧model渲染数据需要

function error_401(err, msg) {
	if (err.status == 401) {
...
// 获取场景id
	...
	var reg=/^[\u4E00-\u9FA5]+$/
/*画布宽高自适应*/
	var canvas_width_lkw = $(window).width() - 62 - 210;//应为60, 但在浏览器缩放时, canvas会掉下去, 可能为不同浏览器缩放时对宽度解读有问题
				...
				if (icon_class == '数 据 集') {
					...
					$.ajax({
						...
						success:function (res) {
							alertmsg('数据文件请求成功',1)
							$('.model2_r_bar1 .model2_r_bar1_cont ul').empty();
							for(var i = 0;i < res.length;i++) {
								$('.model2_r_bar1 .model2_r_bar1_cont ul').append('<li class="check" data-id="'+res[i].id+'" data-obj_id="'+res[i].obj_id+'">'+res[i].file_name+'</li>');
							}
						},
						error:function(err) {
							error_401(err);
					...
					var txt = '可 视 化'
					var type = 'ksh'
				} else if (icon_class == '用户画像') {
					var txt = '用户画像'
					var type = 'hx'
				}
				//右侧tab显示隐藏
				$('.model2_right').children('.model2_r_tabcont[data-type="'+type+'"]').show().siblings('.model2_r_tabcont').hide();
		...
		if (nodeArr[0] != nodeArr[1]) {
			...
			if (!nodeArr[1].inLinks || isonly(nodeArr)) {
				if (nodeArr[0].type == 'sj') {
					if (nodeArr[1].type == 'sf' || nodeArr[1].type == 'hb' || nodeArr[1].type == 'wk' || nodeArr[1].type == 'ksh' || nodeArr[1].type == 'hx') {
						return '正确的连接方式'
				...
				} else if (nodeArr[0].type == 'hx') {
					// if (nodeArr[1].type == 'sj') {
					// 	return '正确的连接方式'
					// }
				} else {
	...
	function nodeClick (node) {
		...
		node.addEventListener('mouseup',function (evt) {
			...
			if (evt.button == 0 && Math.abs(evtx2 - evtx) <= 10 && Math.abs(evty2 - evty) <= 10) {/*判断为非拖拽事件*/
				...
				if (evt.target.type == 'hx') {
					$('.model2_right .model2_r_tabcont[data-type="hx"]').show().siblings().hide();
				}
			} else if (evt.button == 2) {
				$('.fun').hide()
				var top = evt.target.y+50-6
				var left = evt.target.x+124
				// model_right_needId = evt.target._id; //赋值给全局变量, 右侧model渲染数据需要
...
/*数据库弹窗确定事件*/
	$('.databaseSure').click(function () {
		...
		$.ajax({
			url:url_ip + jiekou,
			type:'POST',
			data:{host:server,port:port,username:user,password:password,db_name:name,table_name:file},
			datatype:'json',
			headers:{'Authorization':token},
			beforeSend:function () {
				$('.loading').show()
	...
	function workAjax (now_page) {
		var p = new Promise (function (resolve,reject) {
			...
			for (var i = 0; i < li_L; i++) {
				if (i == 0) {
					list = [['F1',$('.workTop_box li:eq('+i+')').attr('data-id')]]
				} else if (i == 1) {
					var k = i - 1;
...
$(function () {
	...
	// 右侧model输入框获取焦点
	$(".model2_r_bar1 input").focus(function () {
		$(this).nextAll('ul').show();
	})
	
	
	$(".model2_r_bar1 input").blur(function () {
		$(this).nextAll('ul').hide();
	})
	var sf_id = '';
	$(".model2_r_bar1 ul").on('mousedown', 'li', function () {
		$(this).addClass('active').siblings().removeClass('active');
		var nowLiJson = {id: $(this).attr('data-id'), txt: $(this).text()};
		$(".model2_r_bar1 input").val(nowLiJson.txt).attr("data-id", nowLiJson.id);
		sf_id = nowLiJson.id;
		$(this).parent('ul').hide();
	})
	// 右侧model底部按钮点击
	$('.right_bottom_btn_foo .right_bottom_btn').on('click', function () {
		var fooType = $(this).parent().attr('data-type');
		var nowType = $(this).attr('data-type');
		switch(nowType) {
			case 'ck':
				var id_num = model_right_needId
				var nownode = scene.findElements( function (e) { 
					return e._id == id_num
				});
				if (nownode[0].file_data) {
	...
	$('.huaxiangAlert_scroll').on('click', '.huaxiangAlert_dash_r__add_li', function () {
		var preHtml = '<div class="huaxiangAlert_dash_r_li">'+
			'<select name="" id="" class="model_select fl">'+
				'<option value="">用户购物量1</option>'+
				'<option value="">用户购物量2</option>'+
				'<option value="">用户购物量3</option>'+
			'</select>'+
			'<select name="" id="" class="model_select fl">'+
			...
			'</select>'+
			'<p class="p1 model_inputFoo fl hide"><input type="text"></p>'+
			'<p class="p2 model_inputFoo model_inputFoo_s fl"><input type="text" class="fl"><span class="fl">-</span><input type="text" class="fl"></p>'+
		'</div>';
		$(this).before(preHtml);
	})
	//用户画像点击 '添加按钮'
			...
			'<div class="huaxiangAlert_dash">'+
				...
				'<div class="huaxiangAlert_dash_r">'+
					'<div class="huaxiangAlert_dash_r_li">'+
						...
						'<p class="p2 model_inputFoo fl hide"><input type="text">-<input type="text"></p>'+
					'</div>'+
	...
	//用户画像name获取焦点, 显示下拉Ul
	$('.huaxiangAlert_scroll').on('focus', '.huaxiangAlert_inputFoo input', function () {
		$(this).nextAll('ul').show();
	})
	$('.huaxiangAlert_scroll').on('blur', '.huaxiangAlert_inputFoo input', function () {
		$(this).nextAll('ul').hide();
	})
	//用户画像-弹窗打开
	$('.model2_right').on('click', '.model_r_hx_alertBtn', function () {
