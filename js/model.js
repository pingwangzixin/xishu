...
// 保存模板还原
	$(".modelNames p").live('dblclick',function () {
		huanyuan($(this)[0].mid)
	})
	...
	function nodeclick2 (this_node) {
		...
		if (txt == '文件') {
			...
			if (txt == '标准化' || txt == '异常值检测' || txt == '特征选择' || txt == '变量离散化' || txt == '多项式特征' || txt == '标签编码' || txt == '主成分分析' || txt == '因子分析' || txt == '独热编码' ) {
				$('.model_form1_ul1 .right_bottom_btn').css('width','30%')
				$('.model_form1_ul1 .btn3').css('left','70%')
				$('.model_form1_ul1 .btn2').show();
			} else {
				$('.model_form1_ul1 .right_bottom_btn').css('width','45%')
				$('.model_form1_ul1 .btn3').css('left','55%')
				$('.model_form1_ul1 .btn2').hide();
			}
			$('.model_form1_ul1').css('display','block');
			$('.model_form1_ul2').css('display','none');
			$('.model_form1_btn.fl').click(function () {
				$('.model_form1_ul1').css('display','block');
				$('.model_form1_ul2').css('display','none');
			})
			$('.model_form1_btn.fr').click(function () {
				$('.model_form1_ul1').css('display','none');
				$('.model_form1_ul2').css('display','block');
				if (txt == '标准化') {
...
// msgbox信息栏
	...
	$('.msgbox2 div img:eq(0)').click(function () {
		if (msgbox_toggle) {
			...
			$('.msgbox2').css({height:'21.76vh',zIndex:999})
		} else {
			$(this).css('transform','rotate(0deg)')
			$('.msgbox2').css({height:'5.44vh',zIndex:0})
		}
...
// 右边
	// 右边点击关闭按钮
	$('.model_close_btn img').mouseenter(function () {
		$(this).attr('src','img/close2.png');
		$('.model_close_btn img').mousedown(function () {
			$(this).attr('src','img/close3.png');
		});
		$('.model_close_btn img').mouseup(function () {
			sessionStorage.removeItem('model_id_index');
			window.location.href = 'index.html'
		});
		$('.model_close_btn img').mouseleave(function () {
